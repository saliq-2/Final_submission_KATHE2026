...
cdef class IndicProcessor:
    ...
    def __cinit__(self, bint inference=True):
        ...
        # Currency marker (symbol or abbreviation) glued to its number, e.g.
        # "Rs 2,00,000", "Rs.1000", "$140", "US$140" -- wrapped as ONE unit so
        # the currency marker passes through verbatim instead of being left
        # to the model to freely translate/hallucinate (observed: "Rs" was
        # inconsistently rendered as "₹" in one sentence and left as literal
        # "Rs" untouched in another, for the identical pattern).
